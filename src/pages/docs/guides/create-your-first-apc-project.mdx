...
## 5. Add curated project memory if needed
...
Your IDE, CLI, or APX daemon should keep raw runtime state in its own local storage. APX, for
example, keeps sessions, conversations, messages, local runtime memory, and project runtime files
under `~/.apx/projects/<project-id>/`. If a session produces durable project knowledge, extract a
short sanitized fact into APC memory or docs.
