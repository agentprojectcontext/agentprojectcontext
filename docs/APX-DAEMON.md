...
```json
{
  ...
  "log_level": "info",
  "projects": [
    { "path": "/Users/me/work/agency-a", "db": "/Users/me/work/agency-a/.apc/project.db" },
    { "path": "/Users/me/work/agency-b", "db": "/Users/me/work/agency-b/.apc/project.db" }
  ],
  "telegram": {
    "enabled": false,
    "bot_token": "",
...
Lives at `<project>/.apc/project.db`. This SQLite schema is an APX implementation detail layered on top of APC's filesystem contract. The daemon opens one connection per registered project and keeps them warm.
...
| Method | Path | Purpose |
|---|---|---|
| `GET`  | `/projects/:pid/agents/:slug/memory` | Read memory.md |
...
API keys for providers come from `~/.apx/config.json` (`engines.<provider>.api_key`) or env vars (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`). Conversation files live at `.apc/agents/<slug>/conversations/YYYY-MM-DD-NN.md` — append-only markdown is source-of-truth, the SQLite `conversations` + `conversation_turns` tables are a regenerable cache.
...
1. Create an APC session file: `.apc/agents/<slug>/sessions/<YYYY-MM-DD-NN>.md` with `status: 🔄 En progreso` and `runtime: <id>` in frontmatter.
